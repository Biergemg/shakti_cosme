        ...
        <div className="flex min-h-[80px] items-center justify-between gap-6">
          ...
          <nav className="hidden items-center gap-8 text-[0.78rem] uppercase tracking-[0.18em] text-[var(--color-text-muted)] lg:flex">
            {navigationItems.map((item) => (
              <a key={item.href} href={item.href} className="hover:text-[var(--color-brand-strong)]">
                {item.label}
              </a>
            ))}
          </nav>
          ...
          <div className="hidden items-center gap-5 lg:flex">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-2 text-sm text-[var(--color-text-muted)] hover:text-[var(--color-brand-strong)]"
            >
